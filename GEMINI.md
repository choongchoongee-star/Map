...
### 3.5. 딥 링크 및 제휴 연동 (수익화 기반)
- **App Handoff:** 모바일 브라우저에서 네이버 지도 앱으로 자동 전환.
- **초대 링크 공유:** 세션별 고유 URL을 생성하여, 링크 클릭 시 별도의 코드 입력 없이 즉시 리스트에 참여 가능.
- **제휴 서비스 연동 (Affiliate):** 식당 상세 정보 내 캐치테이블, 네이버 예약, 스테이폴리오 등 외부 서비스 딥링크 버튼 제공.
- **수익화 투명성:** 유저가 인식할 수 있도록 제휴 링크 옆에 '수수료 발생 가능' 안내 문구 노출.
...
- [x] 외부 예약 서비스(캐치테이블, 네이버 예약 등) 딥링크 연동 (완료)

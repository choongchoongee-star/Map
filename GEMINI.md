...
- **다중 세션 지원:** 
  - **전체 공유 리스트:** 누구나 보고 추가할 수 있는 공용 공간 (삭제 불가).
  - **개인 리스트:** 로그인 사용자만의 비공개 장소 저장소. (필요 시 코드를 통해 친구와 공유 가능)
  - **공유 세션:** 친구를 초대하여 특정 그룹끼리만 공유하는 실시간 협업 리스트.
- **세션 관리 도구 (⚙️):** 
  - 내가 소유하거나 참여 중인 모든 목록 확인.
  - 각 목록의 초대 코드 복사 및 공유 기능.
  - 불필요한 목록 삭제 및 참여 취소 기능.
...
- **내 리스트로 저장:** 전체 공유 리스트나 다른 세션의 장소를 클릭 한 번으로 내 개인 리스트로 복사하는 기능.
...
- **서버사이드 보안 규칙 (Security Rules):** 
  - `database.rules.json`을 통해 비로그인 사용자의 무단 데이터 수정 및 삭제를 원천 차단.
  - 전체 공유 리스트(`session_001`)의 경우 데이터 쓰기만 허용하고 삭제 요청은 거부되도록 서버 단에서 강제.
- **비용 제어 전략:** 

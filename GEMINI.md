...
### 3.1. 사용자 인증 및 개인 세션
- **구글 로그인:** Firebase Auth의 Google Auth Provider를 통한 간편한 사용자 인증 지원.
- **개인 세션:** 로그인 시 사용자별 고유 세션(`user_sessions/uid`)으로 자동 전환되어 개인별 맛집 리스트 관리 가능.
- **비로그인 모드:** 기본적으로 전체 공유 세션(`session_001`)을 사용하며, 모든 사용자가 읽고 추가 가능.
...
- [ ] 개별 모임방(Session) 생성 및 친구 초대 기능
- [x] 장소 복사 및 내 리스트로 저장하기 기능 (완료)
- [ ] 장소별 그룹 채팅 또는 메모 기능
- [ ] 카테고리별 마커 커스텀 아이콘
- [x] 중복 장소 추가 방지 로직 (완료)
